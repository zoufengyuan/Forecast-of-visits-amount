...
class Network(keras.Model):
    def __init__(self):
        ...
        self.embedding_1 = layers.Embedding(int(5e6), 8)
        self.embedding_2 = layers.Embedding(int(5e6), 64)
...
#        self.layer_bn_2=layers.BatchNormalization()
#        
#        self.fc2 = layers.Dense(128)
#        self.layer_bn_3=layers.BatchNormalization()
    ...
    def call(self,input_x,input_hash_1,input_hash_2,train_able = True,mask = None):
        hash_x_1 = self.embedding_1(input_hash_1)
        hash_x_2 = self.embedding_2(input_hash_2)
        #print(hash_x.shape)
        hash_x_1 = tf.reshape(hash_x_1,[-1,hash_x_1.shape[1]*hash_x_1.shape[2]])
        hash_x_2 = tf.reshape(hash_x_2,[-1,hash_x_2.shape[1]*hash_x_2.shape[2]])
        #hash_x = self.g_avg_pool(hash_x)
        #hash_x = tf.reshape(hash_x,[-1,])
        inputs = tf.concat([input_x,hash_x_1,hash_x_2],axis = 1)
        
        inputs = tf.cond(train_able,lambda: self.layer_bn_1(inputs,training = True),lambda: self.layer_bn_1(inputs,training = False))
        #inputs = self.layer_bn_1(inputs,training = True)
        
        
#        self.fc_out_1 = self.fc1(inputs)
#        self.fc_out_1 = tf.cond(train_able,lambda: self.layer_bn_2(self.fc_out_1,training = True),lambda: self.layer_bn_2(self.fc_out_1,training = False))
#        self.fc_out_1 = tf.nn.relu(self.fc_out_1)
#        
#        
#        #x = self.dropout_1(x)
#        self.fc_out_2 = self.fc2(self.fc_out_1)
#        self.fc_out_2 = tf.cond(train_able,lambda: self.layer_bn_3(self.fc_out_2,training = True),lambda: self.layer_bn_3(self.fc_out_2,training = False))
#        self.fc_out_2 = tf.nn.relu(self.fc_out_2)
...
for epoch in range(2):
    for step,(x,hash_x,y) in enumerate(train_db):
        with tf.GradientTape() as tape:
            out,train_inputs = model(x,hash_x[:,:3],hash_x[:,3:])
        ...
        if step %10 == 0:
            ...
            for x,hash_x,y in test_db:
                test_out,test_inputs = model(x,hash_x[:,:3],hash_x[:,3:],train_able = False)

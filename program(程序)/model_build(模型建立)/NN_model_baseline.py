...
source_model_df = pd.read_pickle('..//wide_data(宽表)//merged_history_data.pkl')
...
drop_date_list = ['visit_date_target','visit_date_recent_visit','visit_date_recent_meteorological',
                  'visit_date_recent_air_quality','visit_date_recent_death','disease']

multi_category_list = ['area','visit_gender','visit_age_multi','so2_multi', 'no2_multi',
...
source_model_df = source_model_df.drop(embedding_list,axis = 1)
...
train_db = tf.data.Dataset.from_tensor_slices((train_x,train_hash_category_df,train_y)).shuffle(train_x.shape[0]).batch(1280)
test_db = tf.data.Dataset.from_tensor_slices((test_x,test_hash_category_df,test_y)).shuffle(test_x.shape[0]).batch(1280)
val_db = tf.data.Dataset.from_tensor_slices((test_x[:1000],test_hash_category_df[:1000],test_y[:1000])).shuffle(1000).batch(1280)
...
class Network(keras.Model):
    def __init__(self):
        ...
        self.embedding = layers.Embedding(int(5e6), 16)
        self.g_avg_pool = layers.GlobalAveragePooling1D()
        self.layer_bn=layers.BatchNormalization(axis = 1)
        #self.fc1 = layers.Dense(64,activation = 'relu')
        self.fc1 = layers.Dense(256,activation = 'tanh')#带L2正则项
        #self.dropout_1 = layers.Dropout(0.5)
        #self.fc2 = layers.Dense(32,activation = 'relu')
        self.fc2 = layers.Dense(256,activation = 'tanh')#带L2正则项
        #self.dropout_2 = layers.Dropout(0.5)#dropout
        self.fc3 = layers.Dense(256,activation = 'tanh')
        
        self.fc4 = layers.Dense(196,activation = 'tanh')
        
        self.fc5 = layers.Dense(196,activation = 'tanh')
        
        self.fc6 = layers.Dense(196,activation = 'tanh')
        
        self.fc7 = layers.Dense(128,activation = 'tanh')
        self.fc8 = layers.Dense(128,activation = 'tanh')
        self.fc9 = layers.Dense(128,activation = 'tanh')
        
        self.fc10 = layers.Dense(64,activation = 'tanh')
        self.fc11 = layers.Dense(64,activation = 'tanh')
        self.fc12 = layers.Dense(64,activation = 'tanh')
        
        
        self.fc13 = layers.Dense(32,activation = 'tanh')
        self.fc14 = layers.Dense(32,activation = 'tanh')
        self.fc15 = layers.Dense(32,activation = 'tanh')
        self.fc16 = layers.Dense(1)#若是分类问题需要加激活函数sigmoid或softmax
        
    def call(self,input_x,input_hash,training = None,mask = None):
        hash_x = self.embedding(input_hash)
        #print(hash_x.shape)
        hash_x = tf.reshape(hash_x,[-1,hash_x.shape[1]*hash_x.shape[2]])
        #hash_x = self.g_avg_pool(hash_x)
        #hash_x = tf.reshape(hash_x,[-1,])
        inputs = tf.concat([input_x,hash_x],axis = 1)
        
        inputs = self.layer_bn(inputs)
        self.fc_out_1 = self.fc1(inputs)
        #x = self.dropout_1(x)
        self.fc_out_2 = self.fc2(self.fc_out_1)
        #x = self.dropout_2(x)
        self.fc_out_3 = self.fc3(self.fc_out_2)
        self.fc_out_4 = self.fc4(self.fc_out_3)
        
        self.fc_out_5 = self.fc5(self.fc_out_4)
        self.fc_out_6 = self.fc6(self.fc_out_5)
        
        self.fc_out_7 = self.fc7(self.fc_out_6)
        self.fc_out_8 = self.fc8(self.fc_out_7)
        self.fc_out_9 = self.fc9(self.fc_out_8)
        self.fc_out_10 = self.fc10(self.fc_out_9)
        self.fc_out_11 = self.fc11(self.fc_out_10)
        self.fc_out_12 = self.fc12(self.fc_out_11)
        
        self.fc_out_13 = self.fc13(self.fc_out_12)
        self.fc_out_14 = self.fc14(self.fc_out_13)
        self.fc_out_15 = self.fc15(self.fc_out_14)
        self.fc_out_16 = self.fc16(self.fc_out_15)
        return self.fc_out_16,inputs
model = Network()
#model.build(input_shape = (None,tmp_train.shape[1]))
#model.summary()
...
for epoch in range(10):
    for step,(x,hash_x,y) in enumerate(train_db):
        with tf.GradientTape() as tape:
            ...
            loss = tf.reduce_mean(tf.losses.MSE(y,out))#MSE
        ...
        if step %10 == 0:
            ...
            for x,hash_x,y in test_db:
                test_out,test_inputs = model(x,hash_x)
                pre = list(np.array(test_out).flatten())
